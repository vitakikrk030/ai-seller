...
function buildState(input = {}, options = {}) {
  ...
  for (const message of messages) {
    ...
    if (expectedAction !== 'none' && messageAt(message.created_at) > messageAt(expectedSince)) {
      followupAttempted = true;
      lastFollowupAt = message.created_at || lastFollowupAt;
      followupActor = actorOf(message) || followupActor;
    }
