...
## 2026-05-15 00:31:00 +03

Усилена работа с фото товаров и добавлена отдельная vision-модель.

Что изменено:

- В `index.js` добавлена отдельная настройка `vision_model`:
  - текстовые диалоги продолжают идти в основную `model`;
  - сообщения с фото теперь могут идти в отдельную, более сильную vision-модель.
- В `requestAi()` добавлен `modelOverride`, чтобы для фото реально использовалась выделенная модель, а не общая текстовая.
- В `compileAiRequest()` добавлен отдельный backend-guard для фото:
  - если точная модель неочевидна, AI дополнительно получает системное правило не выдумывать уверенное название;
  - вместо этого должен использовать `похоже на ...` и предлагать проверку склада.
- Дополнительно усилено правило для спорных fashion-моделей:
  - если есть риск перепутать близкие линейки вроде `Balenciaga 3XL / Track / Runner`,
    AI не должен уверенно называть конкретную линейку.
- В `public/index.html` добавлено отдельное поле `Vision-модель для фото`.
- В `data/ai-seller-control.json` ужесточено правило распознавания фото товара:
  - нельзя уверенно выдумывать точную модель при низкой уверенности;
  - если уверенность не высокая, нужно писать `похоже на ...`;
  - если товара нет в явном каталоге или модель неочевидна, нужно мягко уйти в проверку склада и вернуться позже с ответом.
...
## 2026-05-15 01:12:00 +03

Уточнено жёсткое правило повторного приветствия.

Что изменено:

- В `index.js` system prompt теперь явно сообщает модели окно повторного приветствия из настроек:
  - если клиент писал в пределах последних `N` часов, новое `Привет` или `Здравствуйте` не считается новым первым касанием;
  - в этом случае AI не должен здороваться заново и должен сразу продолжать разговор по сути;
  - если клиент не писал дольше этого окна, диалог можно снова считать новым касанием.
- В `data/ai-seller-control.json` на этапе `first_touch` добавлено отдельное жёсткое правило:
  - новое приветствие клиента внутри активного окна не надо зеркалить повторным приветствием;
  - повторно здороваться можно только после выхода за окно повторного приветствия из настроек.

Результат:

- Правило «через сколько часов можно снова здороваться» теперь зафиксировано явно и в runtime-подсказке, и в правилах AI-продавца.

---

## 2026-05-15 17:24:00 +03

Исправлено дробление реквизитов оплаты и шаблона доставки на несколько пузырей Telegram.

Что изменено:

- В `index.js` усилен post-processing `collapseReplyMessages()`:
  - если AI вернул шаблон оплаты кусками (`Сумма к оплате`, `Способ оплаты`, `Реквизиты`, `Получатель`, `Банк`, `Жду ваш чек`);
  - или шаблон доставки кусками / нумерованным списком;
  - backend теперь принудительно склеивает это в ОДНО сообщение, даже если входящий запрос не был батчем.

Результат:

- Реквизиты оплаты больше не должны уходить построчным спамом в 5-6 пузырей.
- Шаблон доставки тоже должен приходить одним цельным сообщением.

---

## 2026-05-15 17:33:00 +03

Усилен финальный post-payment ответ после получения чека.

Что изменено:

- В блоке `support` (`После оплаты`) в `data/ai-seller-control.json` добавлено обязательное правило:
  - после подтверждения оплаты AI должен не просто сказать `заказ зафиксировал`, а объяснить клиенту, что дальше он ждёт движение в приложении выбранной службы доставки;
  - как только создастся накладная, в приложении этой службы на номер клиента начнут отображаться все этапы движения заказа.
- Добавлен запрет заканчивать post-payment ответ общей фразой вроде `я вам напишу`, если не объяснено, где именно клиент увидит отслеживание.

Результат:

- После оплаты агент должен спокойнее закрывать ожидание клиента и направлять его в приложение выбранной службы доставки, а не оставлять с общим `ждите`.

---

## 2026-05-15 17:46:00 +03

Исправлена потеря клиента после короткого ответа на просьбу измерить стельку.

Что изменено:

- В `shoe_sizing` добавлено отдельное правило:
  - если после запроса длины стельки клиент отвечает коротко и подвешенно (`ок`, `хорошо`, `ага`, `понял`, `щас`, `сейчас`, `позже`, `окей`);
  - AI не должен зависать фразой `жду замер стельки`;
  - это нужно трактовать как мягкое откладывание и самому продолжать диалог.
- Новый обязательный сценарий:
  - дать ориентир по сантиметрам для названного размера;
  - снять страх возвратом;
  - мягко предложить оформить без давления.

Результат:

- На короткое `ок` после вопроса про стельку агент должен не терять клиента, а подхватывать продажу и вести дальше.

---

## 2026-05-15 18:20:00 +03

CRM расширена под реальные продажи: автоподстановка телефона, история заказов и сумма оплаченных покупок.

Что изменено:

- Добавлена таблица `orders` для фиксации заказов по клиенту и чату:
  - сумма,
  - статус оплаты,
  - снимок данных заказа,
  - время создания и оплаты.
- Телефон теперь автоматически синхронизируется в `customers.phone`:
  - из Telegram contact,
  - из обычного текста клиента, если в сообщении есть номер,
  - из AI facts (`phone`, `phone_number`, `customer_phone`).
- Когда AI или менеджер отправляет реквизиты оплаты, создаётся/обновляется draft заказа.
- Когда клиент присылает чек / `перевел` / `оплатил` или похожий PDF-документ, последний ожидающий заказ помечается как `paid`.
- В CRM-карточке клиента теперь отображаются:
  - количество заказов,
  - количество оплаченных,
  - общая сумма, которую клиент оставил в магазине,
  - сохранённые данные клиента,
  - история заказов со статусом, суммой и snapshot данных.

Результат:

- Карточка CRM стала ближе к реальной клиентской истории, а не только к переписке.
- Телефон больше не должен теряться отдельно от карточки клиента.

---

## 2026-05-16 00:35:00 +03

Добавлен backfill для уже существующей истории продаж в CRM.

Что изменено:

- Добавлен скрипт `scripts/backfill-crm-sales-history.js`.
- Скрипт:
  - подтягивает телефоны в `customers.phone` из старых `customer_facts`, Telegram contact и текстовых сообщений;
  - проходит по старой истории сообщений по чатам;
  - находит старые шаблоны оплаты и создает draft-заказы;
  - находит старые чеки / `перевел` / `оплатил` и помечает соответствующие заказы как `paid`.
- В `package.json` добавлена команда:
  - `npm run backfill:crm-sales`

Результат:

- CRM можно дозаполнить не только новыми продажами, но и уже существующей историей, которая была в базе до внедрения `orders`.

---

## 2026-05-16 12:20:00 +03

Подправлены два косяка в логике общения с клиентами по обуви и фото товара.

Что изменено:

- В `photo_id` добавлено явное правило:
  - если клиент просит дополнительные фото товара, агент не должен обещать, что может их доснять или прислать;
  - вместо этого он спокойно объясняет, что все доступные фото уже есть в каталоге / на сайте / в ресурсах магазина;
  - мягко поясняет, что отдельно доснимать товар менеджер не может, потому что параллельно ведёт другие диалоги и оформления;
  - после этого возвращает клиента к выбору размера или оформлению.

- В `concise` и `shoe_sizing` усилено правило контекста по размеру:
  - если клиент уже назвал размер, нельзя спрашивать размер повторно;
  - если клиент спросил `43 есть?`, и агент подтвердил наличие, размер `43` уже считается зафиксированным в контексте;
  - дальше агент должен спрашивать только недостающую длину стельки или уточнять, знает ли клиент её в сантиметрах.

Результат:

- Агент больше не должен обещать дополнительные фото, которых магазин не отправляет.
- При обуви он должен внимательнее держать контекст и не переспрашивать уже названный размер.

---

## 2026-05-16 12:35:00 +03

Поджата карточка заказа в CRM: добавлены товарные атрибуты без раздувания интерфейса.

Что изменено:

- В правой карточке клиента, в блоке `История заказов`, карточка заказа теперь может показывать:
  - название товара;
  - аккуратную ссылку на товар;
  - компактный thumbnail товара или фото/скрин клиента, если он присылал изображение.

- Логика подбора данных сделана каскадно:
  - сначала берется информация из `order.snapshot`;
  - если там данных не хватает, используются сохраненные `facts` чата;
  - если изображения в заказе нет, используется последнее входящее фото клиента до момента заказа.

- Внешний вид блока сохранен компактным:
  - маленький thumbnail справа;
  - название и ссылка слева;
  - без новых тяжелых секций и без визуального шума.

Результат:

- Карточка заказа стала понятнее: видно, что именно заказывал клиент.
- Появилась быстрая навигация к товару по ссылке.
- Если клиент присылал скрин или фото, оно может отображаться прямо в заказе, но без перегруза интерфейса.

---

## 2026-05-16 12:55:00 +03

Исправлена рассинхронизация CRM-карточки заказа, когда в заказе с несколькими товарами мог подтягиваться старый `product_link`.

Что изменено:

- Выявлена причина:
  - `item` из AI facts в некоторых кейсах сохранялся как строка вида `[object Object],[object Object]`;
  - из-за этого карточка не могла корректно прочитать список товаров заказа;
  - при отсутствии валидного `item` UI откатывался к старому `product_link`, который мог относиться к предыдущему товару клиента.

- В `index.js` изменено сохранение AI facts:
  - массивы и объекты теперь пишутся через `JSON.stringify(...)`, а не через `String(...)`;
  - это сохраняет структуру `item` и других сложных facts для будущих заказов.

- Во фронтенде CRM усилено сопоставление заказа:
  - карточка теперь пытается восстановить список товаров из `order.snapshot.item`;
  - если `item` поврежден, карточка сопоставляет заказ с историей сообщений до момента оплаты и достает позиции из подтверждения заказа;
  - если в заказе несколько товаров, старая одиночная ссылка больше не показывается автоматически;
  - ссылка теперь отображается только если она действительно согласуется с конкретным товаром заказа.

Результат:

- В кейсах вроде `New Balance + Nike` карточка больше не должна сваливаться в старую `Puma`.
- Будущие multi-item заказы должны сохраняться и отображаться стабильнее.

---

## 2026-05-16 17:00:00 +03

Дожата логика Telegram-аватаров в CRM для business-чатов, чтобы имя и фото клиента больше не расходились.

Что изменено:

- Найдена причина:
  - customer identity для business-чатов уже бралась правильно от клиента;
  - но avatar-refresh всё ещё использовал `message.from.id`;
  - в исходящих business-сообщениях это мог быть ID магазина, из-за чего в карточке клиента появлялась аватарка `IWAK`.

- В `index.js` avatar-refresh переведён на ID реального клиента:
  - теперь используется `customerIdentity.id`, а не `message.from.id`;
  - это устраняет повторное перезаписывание аватарки магазинским профилем.

- В `db/postgres.js` добавлен более безопасный setter аватара:
  - можно не только обновить аватарку, но и очистить её, если у клиента нет собственного фото;
  - это лучше, чем оставлять в карточке неправильную аватарку магазина.

- Добавлен скрипт `scripts/repair-customer-avatars.js`:
  - проходит по Telegram-клиентам;
  - подтягивает актуальный avatar по `telegram_user_id`;
  - исправляет или очищает неправильные аватарки.

Результат:

- Новые business-диалоги больше не должны ловить аватар магазина вместо клиента.
- Уже испорченные карточки можно централизованно привести в порядок без ручной чистки.

---

## 2026-05-16 17:12:00 +03

Усилена защита ремонта Telegram-аватаров после обнаружения, что repair-скрипт мог очистить аватарки при недоступном token.

Что изменено:

- В runtime-обновлении аватара `getUserProfilePhotos` теперь возвращает статус:
  - `ok: true` — Telegram ответил, можно обновлять или очищать;
  - `ok: false` — была ошибка / нет token / нет user id, аватар не трогаем.

- В `scripts/repair-customer-avatars.js` token теперь берётся не только из `.env`, но и из `data/runtime-config.json`, как в основном сервере.

- Repair больше не очищает avatar при ошибке Telegram API:
  - такие клиенты считаются `skipped`;
  - очищение делается только если Telegram успешно ответил, что фото у пользователя нет.

- Добавлена команда `npm run repair:crm-avatars`.

Результат:

- Ошибка получения аватара больше не приводит к массовому сбросу фото клиентов.
- Repair можно безопасно запускать повторно на проде.

---

## 2026-05-16 17:35:00 +03

Добавлена базовая CRM-детекция точки зависания диалога без автосообщений клиенту.

Что изменено:

- Добавлена таблица `crm_dialog_state` для вычисленного состояния диалога:
  - текущий этап;
  - что ждём от клиента;
  - с какого момента ждём;
  - точка зависания;
  - уверенность, причина и evidence.

- Добавлен детерминированный детектор `lib/crm-dialog-detector.js`:
  - `Клиент завис на первом касании`;
  - `Клиент завис на размере`;
  - `Клиент завис на оформлении`;
  - `Клиент завис после реквизитов`.

- Детекция работает консервативно:
  - не использует LLM;
  - не отправляет follow-up;
  - если данных недостаточно или ожидание уже закрыто, ставит `none`;
  - учитывает, писал ли уже AI или менеджер после открытого ожидания.

- CRM получила компактный UX:
  - чип стадии в списке диалогов;
  - блок `Точка зависания` в карточке клиента;
  - отображение, что ждём, сколько тишина и было ли повторное касание.

- Сброс истории конкретного диалога теперь очищает и вычисленное состояние `crm_dialog_state`.

- Добавлен backfill-скрипт `npm run backfill:crm-dialog-state`, чтобы просканировать уже существующие диалоги.

Результат:

- Можно безопасно включить “второго CRM-сотрудника” с диагностики: сначала видеть, где клиент завис, без риска случайно писать клиентам.
