...
const DEFAULT_DELIVERY_TRACKING_TEXT = [
  ...
  '',
  'Яндекс Доставка: получатель отслеживает доставку в приложении Яндекс Go или по SMS/уведомлению, если сервис отправит ссылку.',
  '',
  'Ozon: статус смотреть в личном кабинете Ozon в разделе Заказы. Для получения в ПВЗ/постамате нужен код или штрихкод из личного кабинета.',
  '',
  'CDEK/СДЭК: после создания накладной заказ обычно подтягивается в приложении/личном кабинете CDEK по номеру телефона получателя, если этот номер зарегистрирован в CDEK. Также можно отслеживать по номеру накладной, если он есть.',
  '',
  'Почта России: статус обычно доступен в приложении/личном кабинете Почты России по номеру телефона или по трек-номеру, когда отправление оформлено.',
  '',
  'Не выдумывай трек-номер, ссылку отслеживания, дату прибытия или статус. Не обещай, что менеджер обязательно пришлет трек вручную, если это не было сказано в диалоге.',
].join('\n');
...
function shouldFinalizeDeliveryChoice(input = {}) {
  ...
  if (isDeliveryTrackingQuestion(text)) return false;
...
function isDeliveryTrackingQuestion(text = '') {
  const source = String(text || '');
  return /(?:сколько|как\s+долго|по\s+времени|срок|когда\s+прид[её]т|доставк[аиуы]|отслед|статус|трек|накладн)/i.test(source)
    && /(?:достав|отправ|сд[эе]к|cdek|яндекс|ozon|озон|почт|пвз|трек|накладн|статус)/i.test(source);
}

function containsManualTrackingPromise(reply = '') {
  return /менеджер\s+(?:пришл[её]т|отправит|даст|скинет)[\s\S]{0,80}(?:трек|номер|ссылк|данные\s+для\s+отслеж)|(?:после\s+отправк[иы]|после\s+передач[иы])[\s\S]{0,120}менеджер\s+(?:пришл[её]т|отправит|даст|скинет)/i
    .test(String(reply || ''));
}

function finalizeDeliveryTrackingReply(input = {}, reply = '') {
  const finalReply = String(reply || '').trim();
  if (!isDeliveryTrackingQuestion(input?.text)) return finalReply;
  if (!containsManualTrackingPromise(finalReply)) return finalReply;

  const service = extractDeliveryService(input.text) || input?.memoryContext?.slotSnapshot?.deliveryService || '';
  const serviceText = service ? ` ${service}` : ' выбранной службы доставки';
...
function getSmalltalkGuidance(config) {
  ...
  return buildGuidanceSection('Живость общения:', [
    ...
    'Когда отвечаешь по фото/скрину клиента, не звучать как протокол осмотра. Вместо сухого "на фото видно/на изображении" писать по-человечески: "вижу", "да, у вас...", "по фото понял", "тут у вас...".',
...
function getResponseGuardGuidance(config) {
  ...
  return buildGuidanceSection('Проверка ответа:', [
    ...
    parseConfigBoolean(config.response_guard_human_tone_enabled, true)
      && 'Если ответ опирается на фото клиента, не осталась ли сухая формулировка вроде "на фото видно", "на изображении", "визуально наблюдается". Перепиши живо: "вижу у вас...", "по фото понял...", "да, тут...".',
...
const SCENARIO_TEST_DEFINITIONS = {
  ...
  photo_size_real: {
    title: 'Фото бирки и реальные фото',
    defaultMessage: 'Мерила оригинал в магазине, подошли эти. Фото у вас реальные?',
  },
  pickup_fitting_return: {
...
function evaluateScenarioReply(reply, scenario, config = runtimeConfig) {
  ...
  addScenarioCheck(
    ...
    !scenarioTextHasAny(lower, [
      ...
      /на\s+изображении/i,
      /визуально\s+(?:видно|наблюдается|определяется)/i,
      /на\s+фото\s+видно/i,
    ]),
  ...
  addScenarioCheck(
    ...
    scenarioTextHasAny(lower, [
      /\?/,
      /пришлите|напишите|подскажите|уточните|проверьте|понадоб|нужн[ыоа]?|можно\s+(?:оплачивать|оформлять)|оформим|оформляем|после\s+оплаты|чек|скрин/i,
    ]),
  ...
  if (scenario.id === 'photo_size_real') {
    addScenarioCheck(
      checks,
      'real_photo_answered_first',
      'Сначала ответил про реальные фото',
      scenarioTextHasAny(lower, [/фото\s+(?:у\s+нас\s+)?реальн|реальные\s+фото|актуальн.*фот/i]),
      'На вопрос "Фото реальные?" сначала подтвердить фото, а не начинать с просьбы про стельку.'
    );
    addScenarioCheck(
      checks,
      'size_confirmed_with_cm',
      'Размер подтверждён в сантиметрах',
      scenarioTextHasAny(lower, [/41[\s\S]{0,80}(?:26|25[,.]6|26[,.]4)[\s\S]{0,30}см|(?:26|25[,.]6|26[,.]4)[\s\S]{0,30}см[\s\S]{0,80}41/i]),
      'Если на фото/в контексте виден EU 41 и клиент говорит, что подошёл, ответить живо: "Да, вижу у вас 41 размер — это примерно 26 см по стельке."'
    );
    addScenarioCheck(
      checks,
      'no_redundant_insole_question',
      'Не переспрашивает стельку заново',
      !scenarioTextHasAny(lower, [/уточните[\s\S]{0,80}(?:стельк|стоп|сантиметр)|подскажите[\s\S]{0,80}(?:длину\s+стельк|сколько\s+см|стоп[ауы]\s+в\s+см)/i]),
      'Клиент уже показал размер и сказал, что он подошёл; не надо снова просить длину стельки.'
    );
  }
  ...
  if (scenario.id === 'pickup_fitting_return') {
    addScenarioCheck(
      checks,
      'pickup_fitting_allowed',
      'Примерка/проверка при получении разрешены',
      scenarioTextHasAny(lower, [/пример|осмотр|провер/i])
        && scenarioTextHasAny(lower, [/пвз|пункт\s+выдач|получени|курьер/i])
        && !scenarioTextHasAny(lower, [/примерк[аи]?\s+нет|без\s+примерк|нельзя\s+пример|не\s+можете\s+пример/i]),
      'Нужно явно сказать: при получении в ПВЗ или у курьера можно примерить, осмотреть и проверить товар.'
    );
    addScenarioCheck(
      checks,
      'offline_not_pickup_confused',
      'Онлайн-формат не запрещает ПВЗ',
      !scenarioTextHasAny(lower, [/только\s+онлайн[\s\S]{0,80}(?:примерк[аи]?\s+нет|самовывоз[а]?\s+нет|пвз\s+нет)|работаем\s+только\s+онлайн[\s\S]{0,80}пвз\s+нет/i]),
      'Не смешивать отсутствие шоурума с получением в ПВЗ: онлайн-формат означает оформление в диалоге, а не запрет примерки при получении.'
    );
    addScenarioCheck(
      checks,
      'return_soft_present',
      'Возврат/обмен упомянут мягко',
      scenarioTextHasAny(lower, [/возврат|обмен/i])
...
function buildMediaInspectionText(input = {}) {
  ...
  return [
    ...
    'Внутренне можно анализировать "что видно на фото", но клиенту отвечай живым языком: "вижу у вас...", "по фото понял...", "да, тут..."; не пиши канцелярское "на изображении видно".',
    'Если на фото видна обувная бирка с EU-размером и клиент говорит, что этот размер ему подошёл, считай размер достаточным сигналом: подтверди EU-размер живо ("вижу у вас 41 размер") и дай ориентир в сантиметрах по стельке, не проси стельку повторно без противоречия.',
